...
const updateStudentCode = errorHandling.asyncHandler(async (req, res, next) => {
  const { studentCode } = req.body; // Assuming the new national ID is provided in the request body

  const student = await UserModel.findOneAndUpdate(
      { nationalID: req.params.nationalID },
      { studentCode: studentCode }, // Specify the new value for nationalID
 ...
 module.exports = {getStudentByNationalId , updateStudentCode}
