...
const absencePermissions = errorHandling.asyncHandler(
  async (req, res, next) => {
    ...
    if (!student) {
      return res
        .status(400)
        .json({ status: httpStatusText.ERROR, message: "In-valid student Id" });
    }
    ...
    if (absence.length === 0) {
      ...
      return res
        .status(201)
        .json({ status: httpStatusText.SUCCESS, data: { absencePermission } });
    } else {
...
const getPermissions = errorHandling.asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ status: httpStatusText.SUCCESS, data: { permission } });
});

//تصريح جماعي
const allAbsencePermissions = errorHandling.asyncHandler(
  async (req, res, next) => {
    const { dateFrom, dateTo, TakeMeal, notes, TypeOfAbsence, studentIds } =
      req.body;
    ...
    for (const student of housedStudents) {
      ...
      const AllabsencePermission =
        await absencesPermissionModel.findOneAndUpdate(
          { StudentId: _id },
          {
            $set: {
              dateFrom,
